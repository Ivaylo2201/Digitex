...
public record CreateMonitorRequest : CreateRequestBase
{
...
public class CreateMonitorRequestHandler(
    IProductRepository<Monitor> productRepository,
    IFileService<Monitor> fileService) 
    : CreateProductRequestHandlerBase<CreateMonitorRequest, Monitor>(productRepository, fileService)
{
    protected override Monitor CreateProduct(CreateMonitorRequest request, string imagePath)
    {
        return new Monitor
        {

...
public interface IProductService<TProduct, TProjection> where TProduct : ProductBase
{
    ...
    Task<Result<PaginatedResponse<IReadOnlyList<ProductSummaryDto>>>> ListAllAsync(int page, int pageSize, Query<TProduct> query, CurrencyIsoCode currencyIsoCode, CancellationToken cancellationToken = default);
}
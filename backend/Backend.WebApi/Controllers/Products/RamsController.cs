...
public class RamsController(IMediator mediator, IFilterService<RamFiltersDto> filterService) : ControllerBase
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<Results<Ok<RamDto>, NotFound>> GetOneAsync([FromRoute] Guid id, [FromQuery] CurrencyIsoCode currency = CurrencyIsoCode.Eur, CancellationToken cancellationToken = default)
    {
        var result = await mediator.Send(new GetOneRamRequest { Id = id, Currency = currency }, cancellationToken);

...
public class SsdsController(IMediator mediator, IFilterService<SsdFiltersDto> filterService) : ControllerBase
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<Results<Ok<SsdDto>, NotFound>> GetOneAsync([FromRoute] Guid id, [FromQuery] CurrencyIsoCode currency, CancellationToken cancellationToken)
    {
        var result = await mediator.Send(new GetOneSsdRequest { Id = id }, cancellationToken);

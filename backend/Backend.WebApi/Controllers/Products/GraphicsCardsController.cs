...
public class GraphicsCardsController(
    ...
    IFiltersProviderService<GraphicsCardFiltersDto> filtersProviderService) : ControllerBase
{
    [HttpGet("{id:guid}")]
    public async Task<Results<Ok<GraphicsCardDto>, NotFound<ProblemDetails>>> GetOneAsync([FromRoute] Guid id, [FromQuery] string currency, CancellationToken cancellationToken)
    {
    ...
    public async Task<Results<Created, BadRequest>> CreateAsync([FromBody] GraphicsCardCreateDto dto, CancellationToken cancellationToken)
    {
        var result = await productService.CreateAsync(dto.Adapt<GraphicsCard>(), cancellationToken);
    ...
    [HttpPatch("{id:guid}")]
    [Authorize(Roles = nameof(Role.Admin))]
    public async Task<Results<Ok, BadRequest>> UpdateAsync([FromRoute] Guid id, [FromBody] GraphicsCardUpdateDto dto, CancellationToken cancellationToken)
    {
        var result = await productService.UpdateAsync(id, dto.Adapt<GraphicsCard>(), cancellationToken);

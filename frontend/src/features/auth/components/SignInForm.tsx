...
export function SignInForm() {
  ...
  const { components: { signInForm } } = useTranslation();
  ...
  return (
    <Card className="w-full max-w-sm bg-theme-gunmetal text-theme-white">
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          <div className="flex gap-2 items-center">
            <Checkbox id="terms" onCheckedChange={(state) => setValue("rememberMe", state === true)} />
      ...
      <div className="flex justify-center items-center gap-2">
        ...
        <Link to="/auth/sign-up" className="text-theme-white text-sm hover:text-theme-crimson duration-200 transition-colors">
          {signInForm.signUp}
        </Link>
      </div>
    </Card>
